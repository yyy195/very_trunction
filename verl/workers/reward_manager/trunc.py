    ...
    def __call__(self, data: DataProto, cut_batch:dict,  return_dict: bool = False) -> torch.Tensor | dict[str, Any]:
        ...
        if self.flag == True:
            ...
            for i in range(original_batch): 
                ...
                majority_answer_for_group = group_counter.most_common(1)[0][0]
                ...
                for j in range(per_pro_rollout_n):
                    ...
                    response_str_for_current_trajectory = response_content[current_trajectory_index]

                    # 构建用于不一致性奖励的答案集合，只包含当前轨迹在各个截断点的答案
                    cut_answer_set = []
                    for cut_sign in cut_batch_statistic.keys():
                        if j < len(cut_batch_statistic[cut_sign]):
                            cut_answer_set.append(cut_batch_statistic[cut_sign][j])

                    # 计算不一致性奖励
                    consistency_score = consistency_reward(response_str_for_current_trajectory, cut_answer_set)

                    # 计算格式奖励
                    format_score = format_reward(response_str_for_current_trajectory)
